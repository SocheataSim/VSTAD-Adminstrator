...
// Fetch playlists with detailed video count
async function fetchAdminPlaylists() {
    try {
        tableBody.innerHTML = `
            <tr>
        ...
        // Fetch detailed info for each playlist to get video counts
        const detailedPlaylists = await Promise.all(
            playlists.map(async (playlist) => {
                try {
                    const detailResponse = await fetch(`${API_URL}/${playlist.id}`, {
                        headers: {
                            'Authorization': `Bearer ${AUTH_TOKEN}`,
                            'Content-Type': 'application/json'
                        }
                    ...
                    if (detailResponse.ok) {
                        const detailedPlaylist = await detailResponse.json();
                        return {
                            ...playlist,
                            videos: detailedPlaylist.videos || [],
                            video_count: detailedPlaylist.videos ? detailedPlaylist.videos.length : 0
                        };
                    }
                } catch (error) {
        ...
        currentPlaylists = detailedPlaylists.map(playlist => ({
            ...playlist,
            creator_name: adminUser ? (adminUser.username || adminUser.full_name || 'Admin') : 'Admin'
        }));

        console.log('Processed playlists with video counts:', currentPlaylists);
        renderPlaylists(currentPlaylists);
...
async function handleAddVideo() {
    ...
    try {
        ...
        await openVideoModal(currentPlaylistId);
        await fetchAdminPlaylists(); // Refresh to update video counts
        videoSelect.value = '';
        displayFeedback('Video added to playlist successfully!', 'success');
